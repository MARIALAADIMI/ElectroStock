...
@section('content')
    ...
    <div class="card">
        ...
        <form action="{{ isset($editProduit) ? route('produits.update', $editProduit->id) : route('produits.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-row">
                <div class="form-group"><label>Code</label><input type="text" name="code" value="{{ old('code', $editProduit->code ?? '') }}" required></div>
                <div class="form-group"><label>Libellé</label><input type="text" name="libelle" value="{{ old('libelle', $editProduit->libelle ?? '') }}" required></div>
                <div class="form-group"><label>Prix (MAD)</label><input type="number" step="0.01" name="prix" value="{{ old('prix', $editProduit->prix ?? '') }}" required></div>
                <div class="form-group"><label>Quantité</label><input type="number" name="qte" value="{{ old('qte', $editProduit->qte ?? '') }}" required></div>
            </div>
            <div class="form-row">
                <div class="form-group" style="flex: 2;"><label>Description</label><textarea name="description">{{ old('description', $editProduit->description ?? '') }}</textarea></div>
                <div class="form-group" style="flex: 1;">
                    <label>Image</label><input type="file" name="image">
                    @if( isset($editProduit) && $editProduit->image)
                        <img src="{{ asset('storage/'.$editProduit->image) }}" style="width: 50px; margin-top: 10px; border-radius: 5px;">
                    @endif
                </div>
            </div>
    ...
    <div class="card">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            ...
            <form action="{{ route('produits.index') }}" method="GET" style="display: flex; gap: 10px;">
                ...
                @if (request('query')) <a href="{{ route('produits.index') }}" class="btn btn-secondary" style="padding: 8px 15px;">X</a> @endif
            </form>
        </div>
        <table>
            <thead>
                <tr><th>Image</th><th>Code</th><th>Libellé</th><th>Prix</th><th>Stock</th><th>Actions</th></tr>
            </thead>
            <tbody>
                @foreach ($produits as $produit)
                    @php $stockClass = $produit->qte == 0 ? 'stock-rupture' : ($produit->qte < 10 ? 'stock-faible' : 'stock-ok'); @endphp
                    <tr class="{{ $stockClass }}">
                        <td>@if ($produit->image) <img src="{{ asset('storage/'.$produit->image) }}" style="width: 40px; border-radius: 5px;"> @else N/A @endif</td>
                        ...
                        <td>
                            <div class="action-btns">
                                ...
                                <form action="{{ route('produits.destroy', $produit->id) }}" method="POST">@csrf @method('DELETE') <button type="submit" class="btn btn-danger" style="font-size: 12px; padding: 5px 10px;" onclick="return confirm('Supprimer ?')">Supprimer</button></form>
                            </div>
                        </td>
                    </tr>
                @endforeach
        ...
        <div style="margin-top: 20px; text-align: center;">{{ $produits->appends(request()->query())->links() }}</div>
    </div>
@endsection