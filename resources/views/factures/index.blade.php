...
@section('content')
    ...
    <div style="display: flex; justify-content: flex-end; gap: 10px; margin-bottom: 20px;">
        <form action="{{ route('factures.index') }}" method="GET" style="display: flex; gap: 10px;">
            ...
            @if(request('query')) <a href="{{ route('factures.index') }}" class="btn btn-secondary" style="padding: 8px 15px;">X</a> @endif
        </form>
    ...
    <div class="card">
        <table>
            <thead><tr><th>N° Facture</th><th>Client</th><th>CIN</th><th>Date</th><th>Total</th><th>Actions</th></tr></thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <div class="action-btns">
                            <a href="{{ route('factures.show', $facture->id) }}" class="btn btn-info" style="font-size: 12px; padding: 5px 12px;">👁 Voir</a>
                            <a href="{{ route('factures.pdf', $facture->id) }}" class="btn btn-primary" style="font-size: 12px; padding: 5px 12px;">📄 PDF</a>
                        </div>
                    </td>
                </tr>
                @endforeach
        ...
        <div style="margin-top: 20px; text-align: center;">{{ $factures->withQueryString()->links() }}</div>
    </div>
@endsection