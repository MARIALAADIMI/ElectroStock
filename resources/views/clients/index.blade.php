...
@section('content')
    ...
    <div class="card">
        ...
        <form action="{{ isset($editClient) ? route('clients.update', $editClient->id) : route('clients.store') }}"
            method="POST">
            @csrf @if (isset($editClient))
                @method('PUT')
            @endif
            <div class="form-row">
                <div class="form-group"><label>CIN</label><input type="text" name="cin"
                        value="{{ old('cin', $editClient->cin ?? '') }}" required></div>
                <div class="form-group"><label>Nom</label><input type="text" name="nom"
                        value="{{ old('nom', $editClient->nom ?? '') }}" required></div>
                <div class="form-group"><label>Prénom</label><input type="text" name="prenom"
                        value="{{ old('prenom', $editClient->prenom ?? '') }}" required></div>
                <div class="form-group"><label>Téléphone</label><input type="text" name="tel"
                        value="{{ old('tel', $editClient->tel ?? '') }}" required></div>
            </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-btns">
                                <a href="{{ route('clients.index', ['edit' => $client->id]) }}" class="btn btn-warning"
                                    style="font-size: 12px; padding: 5px 10px;">Modifier</a>
                                <form action="{{ route('clients.destroy', $client->id) }}" method="POST">@csrf
